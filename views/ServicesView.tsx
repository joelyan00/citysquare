    ...
    <div className="bg-gray-50 min-h-full">
        ...
        {filteredServices.map(item => (
          <div key={item.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 flex flex-col h-full active:shadow-md transition-shadow group">
            ...
            <div className="p-4 flex flex-col flex-grow">
              <h3 className="font-extrabold text-gray-900 text-base line-clamp-1 mb-1.5">{item.title}</h3>
              ...
              <p className="text-gray-600 text-sm font-medium line-clamp-2 mb-4 flex-grow leading-relaxed">{item.description}</p>
              <button className="w-full bg-brand-50 text-brand-700 text-sm font-extrabold py-3 rounded-xl hover:bg-brand-100 transition-colors">
                联系 {item.contactName}
              </button>
            </div>
          </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-5">
          <div className="bg-white w-full max-w-sm rounded-3xl p-7 relative animate-[fadeIn_0.2s_ease-out] shadow-2xl">
            <h2 className="text-2xl font-black mb-5 text-gray-900">发布新信息</h2>
            <div className="space-y-5">
              <div>
                <label className="block text-sm font-bold text-gray-600 mb-2">分类</label>
                <select className="w-full bg-gray-50 p-4 rounded-xl text-base font-medium border-none focus:ring-2 focus:ring-brand-500 text-gray-800">
