    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div>
        ...
        <textarea 
          ...
          rows={4}
          className="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 font-medium"
          placeholder="简单描述您的服务内容，不用担心文笔，CitySquare会帮您润色..."
        />
      </div>
      <div>
      ...
      >
        {loading ? <div className="animate-spin rounded-full h-5 w-5 border-2 border-white border-t-transparent"/> : <><Sparkles className="mr-2" /> CitySquare 智能润色</>}
      </button>
    </div>
  );
  ...
  const renderStep2 = () => (
    <div className="space-y-6 animate-fadeIn">
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <h3 className="text-xs font-bold text-gray-400 uppercase mb-2">CitySquare 生成预览</h3>
      ...
      <div className="p-5 max-w-lg mx-auto">
        {step < 4 && (
            <div className="flex justify-between mb-8 px-2">
                {[1, 2, 3].map(i => (
                    <div key={i} className={`h-1.5 flex-1 rounded-full mx-1 transition-colors ${i <= step ? 'bg-brand-500' : 'bg-gray-200'}`} />
                ))}
            </div>
        )}
